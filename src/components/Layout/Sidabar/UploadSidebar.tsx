...
const UploadSideBar: React.FC = () => {
    ...
    return (
        <div className="flex flex-no-wrap">
            <div className="w-64 absolute sm:relative bg-gray-100 shadow md:h-full flex-col justify-between hidden sm:flex">
                <div className="px-8 h-screen item-center flex flex-col">
                    <div className="h-16 w-full flex items-center">
                        <h1 className="flex w-full justify-between text-[#0099FF] mt-20 cursor-pointer items-center mb-6 text-3xl font-semibold">アップロード</h1>
                    </div>
                    <div className="mx-auto mt-10 w-full">
                        <div className="flex items-center justify-center w-full h-full">
                            <label htmlFor="dropzone-file" className="flex flex-col items-center justify-center w-full h-auto px-8 py-2 border-2 border-gray-300 border-dashed rounded cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-white-700 hover:gg-gray-300">
                                <div className="flex flex-col items-center justify-center" title="Upload">
                                    <svg className="w-8 h-8 text-black-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"></path></svg>
                                </div>
                    ...
                    <ul className="mt-1">
                        <li tabIndex={0} className="rounded-lg flex w-full justify-between text-black-300 cursor-pointer items-center px-2.5 py-2.5 mb-6 leading-tight bg-white hover:bg-[#0099FF] transition-all hover:bg-[#0099FF] focus:bg-[#0099FF] active:bg-[#0099FF]-50 hover:text-white focus:text-white active:text-white outline-none">
            ...
            <div className={!isSidebarOpen ? "sm:hidden" : "w-64 z-40 absolute bg-gray-800 shadow md:h-full flex-col justify-between transition duration-150 ease-in-out"}
                id="mobile-nav" style={{
                    transform: isSidebarOpen ? 'translateX(0px)' : 'translateX(-260px)',
                    transition: 'transform 0.3s ease',
                }}>
                <button aria-label="toggle sidebar" id="openSideBar" onClick={() => sidebarHandler(true)}
                    className={!isSidebarOpen ? 'h-10 w-10 bg-gray-800 absolute right-0 mt-16 -mr-10 flex items-center shadow rounded-tr rounded-br justify-center cursor-pointer focus:outline-none focus:ring-2 focus:ring-offset-2 rounded focus:ring-gray-800' : 'hidden'}>
                ...
                <button aria-label="Close sidebar" id="closeSideBar" onClick={() => sidebarHandler(false)}
                    className={isSidebarOpen ? 'hidden h-10 w-10 bg-gray-800 absolute right-0 mt-16 -mr-10 flex items-center shadow rounded-tr rounded-br justify-center cursor-pointer text-white' : 'hidden'} >
