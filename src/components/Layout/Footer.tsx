...
const Footer: React.FC<FooterProps> = ({ sendQuestions }) => {
    ...
    const maxLines = 10;
    const [botChooseModalOpen, setBotChooseModalOpen] = useState(false);

    const { ChatHistoryData, addChatHistory } = useChatHistoryContext();
    const { Bot, ActiveChatID, addChatBot } = useChatBotListContext();
        ...
        if (ActiveChatID === null) {
            ...
            if (textareaRef.current) {
                ...
                if (questionFiles.length > 0) {
                    ...
                    const ChatHistory = {
                        id: ChatHistoryData.length,
                        Message: message,
                        Date: MessageTime,
                        Coming: true,
                        ChatBotID: ActiveChatID,
                        UserID: 1
                    }
    ...
    return (
        <>
            <div className="x-full border-t-2">
                <form>
                    ...
                    <div className="flex overflow-y-auto items-center py-2 px-3 bg-gray-50 rounded-lg">
                        ...
                        <svg id="Layer_1_1_" version="1.1" viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 cursor-pointer" onClick={handleFileIconClick}>
                        ...
                        <textarea rows={textLineCount} ref={textareaRef} value={questionText} onKeyDown={(e) => handleKeyDown(e)} onChange={(e) => handleTextArea(e)} className="block mx-4 p-2.5 w-full max-h-100 min-h-11 text-sm text-black bg-white rounded border border-indigo-700 focus:border focus:border-indigo-700 dark:bg-gray-100 dark:border-indigo-700 dark:placeholder-black dark:text-black dark:focus:border-indigo-700 outline-none"
                        ...
                        <button type="button" onClick={handleSendQuestions} className="inline-flex justify-center p-2 text-blue-600 rounded-full cursor-pointer">
                            <div className="flex items-center py-2 px-3 bg-gray-50 rounded-lg dark:bg-white">
                                <div className="inline-flex justify-center p-2 text-blue-600 rounded-full cursor-pointer">
            ...
            <BotChooseModal isBotModalOpen={botChooseModalOpen} onClose={() => setBotChooseModalOpen(false)}>
                <></>
            </BotChooseModal>
        </>
    )
